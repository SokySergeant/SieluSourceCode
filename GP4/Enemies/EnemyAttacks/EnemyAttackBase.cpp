...
void UEnemyAttackBase::DamageTarget()
{
	...
	if(AttackerGrunt)
	{
		//If target is in anger state and enemy is of type sadness, or target is in sadness state and enemy is of type hatred
		if((TargetEmotionComponent->GetEmotionState() && AttackerGrunt->MyEmotion == EEmotionType::Sadness) ||
			(!TargetEmotionComponent->GetEmotionState() && AttackerGrunt->MyEmotion == EEmotionType::Anger)) 
		{
